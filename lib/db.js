...
internals.sync = function sync(method, model, options){
  ...
  switch (method){
    case 'read':
      if (model.id){
        internals.db.get(getId(model), function(err, doc){
          if (err){
            app.log.error('db: read:', {model: model.id, err: err})
            return error(model, err, options)
          }
        ...
        internals.db.view('backbone/collection', {key: getCollectionName(model)}, function(err, res){
          if (err){
            app.log.error('db: read:', {collection: getCollectionName(model), err: err})
            return error(model, err, options)
          }
    ...
    case 'delete':
      ...
      var dbDeleteCb = function(err, res){
          ...
          if (!res.ok){
            ...
            return internals.db.save(model.id, {idDeleted: true}, dbDeleteCb)
          }
        ...
        feed.pause()
      internals.db.save(model.id, {isDeleted: true}, dbDeleteCb)
      break
  }
}

internals.feedSetup = function feedSetup(){
  feed = internals.db.changes({
    filter: function(doc){
...
exports.attach = function(opts){
  ...
  getCollectionName = internals.options.getCollectionName
  getId = internals.options.getId
