    ...
    case 'create': case 'update':
      if (model.get('_rev')){
        ...
        internals.db.get(model.id, function(err, doc){
          var dbUpdateCb = function syncUpdateCb(err, res){
              if (err){
                if (err.error === 'conflict'){
                  ...
                  return internals.sync('update', model, options)
                }
          ...
          if (err) {
            app.log.error('db: update: get error', {model: model.id, err: err})
            return error(err)
          }
          else {
            // overwrite what's in the db with our new info
            // ensure we're using the latest rev from the db
            newModel = _.extend({}, doc, internals.toJSON(model), {_rev: doc._rev, updatedAt: new Date().toString()})

            if (newModel.createdAt !== doc.createdAt) {
              app.log.warn('db: update: createdAt modification rejected', {id: model.id})
              model.set({createdAt: doc.createdAt})
              return error('createdAt can\'t be modified.')
            }
            if (newModel.isDeleted !== doc.isDeleted && options.isDelete !== true) {
              app.log.warn('db: update: isDeleted modification rejected', {id: model.id})
              model.set({isDeleted: doc.isDeleted})
              return error('models can only be marked deleted by the DELETE method')
            }
        ...
        internals.getUUID(function(uuid){
          var id = internals.options.getId(model) + '/' + uuid
            ...
            , newModel = _.extend(model.toJSON(), {createdAt: date, updatedAt: date})
            , dbCreateCb = function syncCreateCb(err, res){
              ...
              if (!res || !res.ok){
                ...
                return internals.sync('create', model, options)
              }
          ...
          if (!_.isUndefined(newModel.isDeleted)) {
            app.log.warn('db: create: isDeleted attr rejected', {id: model.id})
            model.set({isDeleted: void 0})
            return error('models can not be created in a deleted state')
          }
...
internals.feedSetup = function feedSetup(){
  internals.feed = internals.db.changes({
    filter: function feedSetupFilter(doc){
  ...
  internals.feed.on('change', function feedOnChange(change){
    var name = change.doc._id.replace(/(.*?)\/[A-z0-9]*$/, '$1')
      , model

    // bail if we don't know about this collection
    if (!app.collections || !app.collections[name]) {
      app.log.debug('db: changes: ignoring: no collection found', {name: name, id: change.doc._id})
      return
    }

    // do this on nextTick to ensure backbone has time to process
    process.nextTick(function feedOnChangeNextTick(){
      ...
      if (change.deleted && model || (model && change.doc.isDeleted)){
        app.log.debug('db: changes: removing:', {id: change.doc._id})
  ...
  internals.feed.on('error', function feedOnError(err){
